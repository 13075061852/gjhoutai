...
(function () {
  ...
  const refreshAuthUsers = async () => {
    authUsers = await authClient.listUsers();
    App.navigation?.refreshAccess?.();
  };

  const notifyAuthUsersChanged = () => {
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('gjh:auth-users-changed'));
    }
  };
  ...
  const getRolePageOverride = (permissionKey, pageId) => {
    ...
    const productionKey = getPermissionDepartmentKey('生产部');
    const productionLeadKey = getPermissionDepartmentKey('生产部主管');
    if (permissionKey === productionLeadKey) keys.push(productionKey);
    if (permissionKey === productionKey) keys.push(productionLeadKey);
    const override = keys
      .map((key) => rolePagePermissionOverrides?.[key]?.[pageId])
